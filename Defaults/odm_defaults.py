...
ODM Defaults Parser
====================
Parses IBM ODM interview rule files (.m extension) and extracts:
  - Default value rules      → {prefix}_Defaults.csv
  - Relevancy rules          → included in Defaults.csv (Rule Type = Relevancy)
  - Stage rules              → {prefix}_Stage_Rules_Review.csv
...
Usage:
    python3 odm_defaults.py \
        ...
        --input  /path/to/odm/defaults/folder/ \
...
Pipeline position:
    INPUT:  ODM .m files (defaults/relevancy/stage rules)
    OUTPUT: Defaults.csv — reviewed and confirmed clean CSV
    NEXT:   odm-defaults-classify → System_Defaults.csv + Business_Defaults.csv
"""
...
# Import shared core — must be in same directory or PYTHONPATH
import sys
...
# ── Attribute Extraction (defaults-specific) ──────────────────────────────────

def extract_attributes(content: str) -> dict:
    """
    Extract interview attributes from the then block.
    These are the defaults-specific fields — InterviewAttributeType.*
    Not present in validation or instruction files.
    """
    attrs = {}
...
def _parse_default_value(raw: str) -> str:
    ...
    Priority order — most specific first:
      YearsAtAddress date patterns
      today's year calculations
      PurchaseDate patterns
      PropertyAddress field references
      Generic field references
      Calculated date fallback
      Raw string fallback
    """
    def field_ref(r):
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Parse ODM defaults/relevancy/stage rules into CSV')
